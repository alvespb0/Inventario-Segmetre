...
@section('title', 'Fornecedores | Edição de Fornecedor')
...
@section('content')
    ...
    <section class="card card-center" style="padding: 1.25rem; max-width: 720px; width: 100%;">
        <header style="margin-bottom: 1rem; display:flex; align-items:center; justify-content: space-between; gap:.75rem;">
            <div>
                <h1 style="margin:0; font-size: 1.25rem;">Editar Fornecedor</h1>
        ...
        @endif
        <form method="post" action="{{route('fornecedores.update', $fornecedor->id)}}" style="display:grid; gap: 1rem; text: align-center">
            @csrf

            <div style="display:grid; gap:.5rem;">
                <label for="nome" style="font-weight:600;">Nome do Fornecedor</label>
                <input id="nome" name="nome" type="text" class="input" placeholder="Ex.: Lojinha do tião" required style="width:100%;" value="{{$fornecedor->nome}}" />
            </div>
            <div style="display:grid; gap:.5rem;">
                ...
                <input id="cnpj" name="cnpj" type="text" class="input" placeholder="Ex.: 06080215000146" required style="width:100%;" value="{{$fornecedor->cnpj}}"/>
            </div>
            <div style="display:grid; gap:.5rem;">
                <label for="cliente_segmetre" style="font-weight:600;">É cliente Segmetre?</label>
                <div style="display: inline-flex; align-items: center; gap: 6px;">
                    <input type="radio" name="cliente_segmetre" id="cliente_segmetre" value="1" {{$fornecedor->cliente_segmetre ? 'checked' : ''}}>
                    <label for="cliente_segmetre" style="font-weight: 600; margin: 0;">Sim</label>
                         <input type="radio" name="cliente_segmetre" id="cliente_segmetre" value="0" {{!$fornecedor->cliente_segmetre ? 'checked' : ''}}>
                    <label for="cliente_segmetre" style="font-weight: 600; margin: 0;">Não</label>
                </div>
            </div>
            <div style="display:flex; gap:.5rem; justify-content:flex-end; margin-top:.5rem;">
                <a href="/fornecedores/excluir/{{$fornecedor->id}}" class="btn btn-ghost" type="button">Excluir</a>

...
@section('title', 'Item | Lançar Item no Setor')
...
@section('content')
    ...
    <section class="card card-center" style="padding: 1.25rem; max-width: 720px; width: 100%;">
        <header style="margin-bottom: 1rem; display:flex; align-items:center; justify-content: space-between; gap:.75rem;">
            <div>
                <h1 style="margin:0; font-size: 1.25rem;">Lançar Item no Setor</h1>
                <p style="margin:.25rem 0 0; color:#cfe8f0;">Informe o item e a quantidade em estoque.</p>
            </div>
        </header>
        @if ($errors->any())
        ...
        @endif
        <form method="post" action="{{route('itemSetor.create')}}" style="display:grid; gap: 1rem; text: align-center">
            @csrf
            ...
            <div style="display:grid; gap:.5rem;">
                <label for="qtd_estoque" style="font-weight:600;">Quantidade em Estoque</label>
                <input id="qtd_estoque" name="qtd_estoque" type="number" min=0 class="input" placeholder="Ex.: 5" required style="width:100%;" />
            </div>

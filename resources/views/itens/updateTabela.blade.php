...
@section('content')
    <section class="card" style="padding: 1.25rem;">
        <header style="display:flex; justify-content: space-between; align-items:center; gap:1rem; margin-bottom: .75rem;">
            ...
            <div id="itens-fornecedor-list" class="table-like">
                <div class="table-row table-head" style="display:grid; grid-template-columns: 80px 1fr 1.5fr 1.5fr 140px 180px; gap:.5rem; padding:.6rem .75rem; border-bottom:1px solid rgba(254,252,251,.08); color:#cfe8f0;">
                ...
                @forelse($itemFornecedor as $item)
                    <div class="table-row" style="display:grid; grid-template-columns: 80px 1fr 1.5fr 1.5fr 140px 180px; gap:.5rem; align-items:center; padding:.6rem .75rem; border-bottom:1px solid rgba(254,252,251,.06);">
                        <div>#{{ $item->id }}</div>
                        ...
                        <div style="display:flex; justify-content:flex-end;">
                            <form method="POST" action="{{route('itens.fornecedores-atualiza-valor')}}" style="display:flex; gap:.5rem; width:100%; max-width:160px;">
                                @csrf
                ...
                @empty
                    <div class="card" style="padding: .9rem;">Nenhum Fornecedor Vinculado.</div>
                @endforelse
            </div>
    </section>
@endsection

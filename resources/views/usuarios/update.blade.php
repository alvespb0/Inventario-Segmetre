...
@section('title', 'Usuários | Edição de Usuário')
...
@section('content')
    ...
    <section class="card card-center" style="padding: 1.25rem; max-width: 720px; width: 100%;">
        <header style="margin-bottom: 1rem; display:flex; align-items:center; justify-content: space-between; gap:.75rem;">
            <div>
                <h1 style="margin:0; font-size: 1.25rem;">Editar Usuario</h1>
        ...
        @endif
        <form method="post" action="{{route('usuarios.update', $usuario->id)}}" style="display:grid; gap: 1rem; text: align-center">
            @csrf
            <div style="display:grid; gap:.5rem;">
                <label for="nome" style="font-weight:600;">Nome do Usuário</label>
                <input id="nome" name="nome" type="text" class="input" placeholder="Ex.: João da Silva Sauro" required style="width:100%;" value="{{$usuario->nome}}"/>
            </div>
            <div style="display:grid; gap:.5rem;">
                <label for="login" style="font-weight:600;">Login do Usuário</label>
                <input id="login" name="login" type="text" class="input" placeholder="Ex.: Joao.Silva" required style="width:100%;" value="{{$usuario->login}}"/>
            </div>
            <div style="display:grid; gap:.5rem;">
                <label for="email" style="font-weight:600;">Email do Usuário</label>
                <input id="email" name="email" type="text" class="input" placeholder="Ex.: joao@example.com.br" required style="width:100%;" value="{{$usuario->email}}"/>
            </div>
            <div style="display:grid; gap:.5rem;">
            ...
            <div style="display:grid; gap:.5rem;">
                ...
                <select name="setor_id" id="setor" required style="width:100%;">
                    @foreach($setores as $setor)
                        <option value="{{$setor->id}}" {{$usuario->setor_id == $setor->id ? 'selected' : ''}}>{{$setor->nome}}</option>
                    @endforeach
                </select>
            </div>
            <div style="display:grid; gap:.5rem;">
                ...
                <div style="display: inline-flex; align-items: center; gap: 6px;">
                    <input type="radio" name="is_administrator" id="administrator" value="1" {{$usuario->is_administrator ? 'checked' : ''}}>
                    <label for="administrator" style="font-weight: 600; margin: 0;">Administrador</label>
                         <input type="radio" name="is_administrator" id="administrator" value="0" {{!$usuario->is_administrator ? 'checked' : ''}}>
                    <label for="administrator" style="font-weight: 600; margin: 0;">Comum</label>
                </div>
            </div>
            <div style="display:flex; gap:.5rem; justify-content:flex-end; margin-top:.5rem;">
                <a href="/usuarios/excluir/{{$usuario->id}}" class="btn btn-ghost" type="button" onclick="return confirm('Tem certeza que deseja excluir esse usuário?')">Excluir</a>
